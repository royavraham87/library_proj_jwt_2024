...
def loan_book():
    if request.role == 'customer':
        ...
        if book and book.status == 'available':
            ...
            new_loan = Loan(user_id=request.user_id, book_id=book.id, loan_date=loan_date, return_date=return_date)
            book.status = 'on loan'
    ...
    if request.role == 'customer':
        ...
        if loan:
            ...
            if datetime.strptime(loan.actual_return_date, '%Y-%m-%d %H:%M:%S') > datetime.strptime(loan.return_date, '%Y-%m-%d %H:%M:%S'):
                ...
                late_loan = LateLoan(loan_id=loan.id, user_id=loan.user_id, book_id=loan.book_id, late_days=late_days)
...
def get_late_loans():
    if request.role == 'admin':
        loans = Loan.query.filter(Loan.return_date < format_datetime(get_local_time()), Loan.actual_return_date == None).all()
        return jsonify([loan_to_dict(loan) for loan in loans])
    else:
        return jsonify({"message": "Unauthorized"}), 403
